...
export class UserWorkoutExerciseSetService {
   ...
   * Új set hozzáadása.
   */
  addSet(userWorkoutExerciseId: number): Observable<any> {
   ...
   * Egy set módosítása.
   */
  updateSet(id: number, data: Partial<UserWorkoutExerciseSetModel>): Observable<any> {
    return this.http.put(`${API_ENDPOINTS.userWorkoutExerciseSets}/${id}`, data);
  }
  ...
  deleteSet(id: number): Observable<any> {
    return this.http.delete(`${API_ENDPOINTS.userWorkoutExerciseSets}/${id}`);
  }
}
