...
export class CoachExercisesBoardComponent implements OnInit, OnChanges {
  ...
  nextExercisePage(): void {
    if (this.exercisePage < this.totalExercisePages) {
      this.exercisePage++;
    }
  }
  ...
  isExerciseSelected(ex: Exercise): boolean {
    return !!this.selectedExercises.find((exercise) => exercise.id === ex.id);
  }
  ...
  toggleExerciseSelection(ex: Exercise, checked: boolean): void {
    /**
    ...
    if (checked) {
      if (!this.selectedExercises.find((exercise) => exercise.id === ex.id)) {
        this.selectedExercises.push(ex);
      }
    } else {
      this.selectedExercises = this.selectedExercises.filter((exercise) => exercise.id !== ex.id);
    }
  ...
  onCheckboxChange(ex: Exercise, event: Event): void {
    /**
